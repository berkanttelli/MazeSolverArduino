...
void encoder() {
  int i = 0;
  while (i < 150) {
    if ((digitalRead(sensorsol1) == 0) and (digitalRead(sensorsag1) == 1)) {
      motor(55, 45);
    }
    else if ((digitalRead(sensorsag1) == 0) and (digitalRead(sensorsol1) == 1)) {
      motor(45, 55);
    }
    else {
      motor(50, 52);
    }
    okunanAc1 = digitalRead(Ac1);
    if (okunanAc1 == 0) {
      while (okunanAc1 == 0) {
        okunanAc1 = digitalRead(Ac1);
      }
